namespace EasyHash.MSIL
{
    ...
    internal static class EasyHashUsingDynamicAssembly<T>
    {
        private static Func<T, int> defaultHasher;
        private static Func<T, int> hasher;
        private static Exception initializationException;

        static EasyHashUsingDynamicAssembly()
        {
            Reset();
        }
        ...
        internal static void Reset()
        {
            try
            {
                hasher = defaultHasher = new DynamicAssemblyGetHashCodeGenerator<T>().Build();
        ...
        public static void Register([NotNull]Action<GetHashCodeConfiguration<T>> configure)
        {
            try
            {
                ...
                Func<T, int> hashFn = new DynamicAssemblyGetHashCodeGenerator<T>().Build(configuration);
